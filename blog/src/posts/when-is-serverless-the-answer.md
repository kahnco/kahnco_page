...
다만 서버리스는 **"서버가 없다"가 아니라 "서버를 내가 신경 쓰지 않는다"**입니다. 서버는 여전히 어딘가에서 돌고, 그 추상화의 대가로 cold start·실행시간 제한·비용 구조 같은 새로운 제약이 따라옵니다. 그래서 서버리스는 "기본값으로 깔고 보는" 기술이 아니라, **워크로드마다 따져봐야 하는 선택지**입니다.
