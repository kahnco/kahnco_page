...
"순서대로 읽으면 한 주제를 이해할 수 있는" 글 묶음은 `blog/src/lib/series.ts` 의 `SERIES` 로 정의합니다.
- 각 편에 `slug` 이 있으면 작성된 글로 링크되고, 없으면 "예정"으로 흐리게 표시됩니다.
- 글이 어느 시리즈에 속하는지는 `series.ts` 의 slug 목록으로 정해지므로 **frontmatter 설정이 필요 없습니다.**
- 시리즈에 속한 글에는 상단 배지(N/총편)와 하단 커리큘럼 박스가 자동으로 붙습니다.

새 글을 써서 커리큘럼의 예정 편을 채울 때는, `series.ts` 의 해당 `part` 에 `slug` 만 추가하면 됩니다.
